...
WATCHDOG_CPU_TEMP_WARN="${WATCHDOG_CPU_TEMP_WARN:-65.0}"
WATCHDOG_CPU_TEMP_ERROR="${WATCHDOG_CPU_TEMP_ERROR:-75.0}"
...
check_cpu_temperature() {
    ...
    if (( $(awk "BEGIN {print ($cpu_temp >= $WATCHDOG_CPU_TEMP_ERROR)}") )); then
        log_error "CPU temperature critical: ${cpu_temp}°C (threshold: ${WATCHDOG_CPU_TEMP_ERROR}°C)"
    ...
    # Check against warning threshold
        ...
        log_warn "CPU temperature high: ${cpu_temp}°C (threshold: ${WATCHDOG_CPU_TEMP_WARN}°C)"
